  ...
  this.map = function(indices) {
    var request = $http({
      ...
      if(status === 0) {
        ...
        alertSrv.set('Error',"No index found at "+config.elasticsearch+"/"+indices.join(',')+"/_mapping. Please create at least one index. if you are using a proxy ensure it is configured correctly.",'error');
      }
    });
